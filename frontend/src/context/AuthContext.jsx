...
export const AuthProvider = ({ children }) => {
  ...
  // UPDATE USER - updates local state and optionally syncs with backend
  const updateUser = async (updates) => {
    try {
      // Update local state immediately for better UX
      setUser(prev => ({ ...prev, ...updates }));

      // If updates include fields that need backend sync, make API call
      const backendFields = ['first_name', 'last_name', 'age', 'gender', 'bio', 'city'];
      const needsBackendSync = Object.keys(updates).some(key => backendFields.includes(key));

      if (needsBackendSync) {
        const res = await axios.put('/api/users/update-profile', updates, axiosOpts);
        if (res?.data?.user) {
          setUser(res.data.user);
        }
      }
      ...
      // Revert to previous state by fetching from server
